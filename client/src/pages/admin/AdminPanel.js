...
const AdminPanel = () => {
    ...
    useEffect(() => {
        handleSearch(); // Call handleSearch whenever the search input changes
    }, [search, users]);
    ...
    const fetchUsers = async () => {
        try {
            const response = await axios.get(`${apiUrl}/api/users`);
            setUsers(response.data);
            setFilteredUsers(response.data); // Initially, all users are displayed
        } catch (error) {
            toast.error('Failed to fetch users');
        }
    };
            ...
            <div className="min-h-screen bg-gradient-to-r from-blue-50 to-blue-100 p-8">
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <Tab.Group>
                        ...
                        <Tab.Panels>
                            <Tab.Panel>
                                <UsersTable users={filteredUsers} handleEdit={handleEdit} handleDelete={handleDelete} />
                            </Tab.Panel>
                            <Tab.Panel>
                                <SetPassword />
                            </Tab.Panel>
