...
const UserDashboard = () => {
  ...
  const truncateTask = (task) => {
    ...
    return words.length > 3 ? `${words.slice(0, 3).join(' ')}...` : task;
  };
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {currentEntries.map((entry, index) => (
                                <tr key={index}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{moment(entry.date).format('DD-MM-YYYY')}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{entry.hours}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{truncateTask(entry.task)}</td>
